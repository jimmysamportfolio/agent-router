...
export interface IScanRepository {
  getByReviewId(reviewId: string): Promise<ScanResultOutput | undefined>;
}
...
export class ScanRepository extends BaseRepository implements IScanRepository {
  async getByReviewId(reviewId: string): Promise<ScanResultOutput | undefined> {
    const row = await this.queryOne<ScanRow>(SCAN_QUERY_SQL, [reviewId]);
    if (!row) return undefined;

    const violations = await this.query<ViolationRow>(VIOLATIONS_QUERY_SQL, [
      reviewId,
    ]);

    const result: ScanResultOutput = {
      review: {
